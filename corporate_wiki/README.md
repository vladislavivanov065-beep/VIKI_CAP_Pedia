...
По умолчанию — небольшие модели (эмбеддинги ~470 МБ, генеративная ~1 ГБ),
рассчитанные на работу на CPU без GPU; при необходимости замените на
более крупные модели, если у сервера есть ресурсы (крупнее генеративная
модель точнее следует инструкции «отвечай только из текста статьи», но
генерирует ответ заметно дольше). Если веса скачать не удалось (нет
сети, модель недоступна) — на странице «Локальный ИИ» появится текст
ошибки, а обучение не отметится как выполненное; ответы на вопросы при
этом продолжают работать через обычный поиск по тексту статьи.

Если после генерации ответ почти не пересекается словами с найденными
фрагментами статей (модель "выдумала" ответ вместо того, чтобы
пересказать текст) — такой ответ отбрасывается автоматически, и вместо
него используется обычный поиск по тексту статьи.
