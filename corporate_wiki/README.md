...
Требует `OPENAI_API_KEY` в `.env`. Новые и изменённые статьи
переиндексируются автоматически при сохранении — команда ниже нужна
только один раз, сразу после того как вы впервые задали ключ (чтобы
проиндексировать уже существующие статьи), либо для восстановления,
если таблица эмбеддингов повреждена. В entrypoint она не запускается
автоматически при каждом старте контейнера — иначе каждый перезапуск
заново оплачивал бы переиндексацию всех статей без необходимости.
