...
def test_create_initial_admin_creates_user_from_settings(settings):
    ...
    admin = User.objects.get(email="bootstrap@example.com")
...
def test_invalidate_user_sessions_command_terminates_sessions():
    ...
    call_command("invalidate_user_sessions", user.email)

    assert not SessionStore().exists(session.session_key)


def test_invalidate_user_sessions_command_unknown_email_raises():
    with pytest.raises(CommandError):
        call_command("invalidate_user_sessions", "nobody@example.com")
